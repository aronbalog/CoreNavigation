extension Destination {
    ...
    public func viewController(_ block: @escaping (Self.ViewControllerType) -> Void, _ failure: ((Error) -> Void)? = nil) {
        ...
        Navigator(queue: queue).resolve(with: builder.configuration, onComplete: { (_, viewController, _) in
    ...
    func resolvedDestination(_ block: @escaping (Self) -> Void, _ failure: ((Error) -> Void)? = nil) {
        ...
        Navigator(queue: queue).resolve(with: builder.configuration, onComplete: { (destination, _, _) in
            block(destination)
        }) { failure?($0) }
    }
